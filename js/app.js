<!DOCTYPE html>
<html lang="de">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover">
  <title>Abrechnungsprogramm</title>

  <!-- OCR -->
  <script src="https://cdn.jsdelivr.net/npm/tesseract.js@5/dist/tesseract.min.js"></script>

  <!-- PDF.js für Gutschrift -->
  <script src="https://cdn.jsdelivr.net/npm/pdfjs-dist@3.11.174/build/pdf.min.js"></script>
  <script>
    pdfjsLib.GlobalWorkerOptions.workerSrc =
      "https://cdn.jsdelivr.net/npm/pdfjs-dist@3.11.174/build/pdf.worker.min.js";
  </script>

  <!-- XLSX (SheetJS) für Gutschrift (Excel-Import) -->
  <script src="https://cdn.jsdelivr.net/npm/xlsx@0.18.5/dist/xlsx.full.min.js"></script>

  <link rel="stylesheet" href="css/styles.css">
  <script defer src="js/app.js?"></script>

</head>

<body>
<div class="container">

<header>
  <h1>📦 Abrechnungsprogramm</h1>
  <small><b>Bismillah</b> – Im Namen Allahs, des Allerbarmers, des Barmherzigen</small>
</header>

<!-- ================== PAKETE ================== -->
<div class="section section-pkg">
  <div class="accordion" onclick="togglePkg()" aria-expanded="false">📦 Pakete verwalten</div>
  <div id="pkgContent" class="accordion-content">

    <div class="row">
      <label>Name</label>
      <input id="pn" placeholder="z.B. Paket A">
    </div>

    <div class="row">
      <label>Preis €</label>
      <input id="pp" type="number" inputmode="decimal" placeholder="z.B. 12.50">
    </div>

    <div class="row">
      <label>Keywords</label>
      <input id="pk" placeholder="kommagetrennt, z.B. Apfel,Banane">
    </div>

    <div class="row" style="gap:10px;">
      <div style="flex:1">
        <label>Symbol</label>
        <button id="symbolBtn" type="button" onclick="openEmojiPicker()">📦</button>
      </div>
      <div style="flex:1">
        <label>Farbe</label>
        <input id="pc" type="color" value="#22c55e">
      </div>
    </div>

    <div class="work-controls" role="group" aria-label="Pakete Aktionen">
  <button class="primary" onclick="addPackage()">➕ Paket</button>
  <button class="ghost" onclick="exportPackages()">⬇ Pakete exportieren</button>

  <input type="file" id="importFile" accept=".json" style="display:none"
         onchange="importPackages(this.files)">
  <button class="ghost" onclick="document.getElementById('importFile').click()">⬆ Pakete importieren</button>
</div>


    <div class="table-wrap"><table id="pkgTable"></table></div>
  </div>
</div>

<!-- ================== ARBEIT (1–3) ================== -->

<div class="section section-work">
  <div class="accordion" onclick="toggleWork()" aria-expanded="false">🧾 Arbeit (1–3)</div>

  <div id="workContent" class="accordion-content" style="display:none">
    <!-- Arbeit Controls (sticky) -->
    <div class="work-controls" role="group" aria-label="Arbeit Aktionen"> 
      <button class="danger" onclick="resetWork()">♻ Reset</button>
      <button class="ghost" onclick="saveWork()">⬇💾 Save</button>

      <input type="file" id="importWorkFile" accept=".json" style="display:none"
             onchange="loadWork(this.files)">
      <button class="ghost" onclick="document.getElementById('importWorkFile').click()">⬆📁 Load</button>
    </div>


    <!-- 1️⃣ Screenshots -->
    <div class="inner-block">
      <div class="section-title">
        <h2>1️⃣ Screenshot hochladen</h2>
        <span class="badge">JPG/PNG</span>
      </div>

      <label class="upload-card">
        <input class="file-input" type="file" multiple accept="image/jpeg,image/png,image/heic,image/heif"
               onchange="loadScreenshots(this.files)">
        <div class="upload-card-inner">
          <div class="upload-icon">⬆️</div>
          <div>
            <div class="upload-title">Bilder auswählen</div>
            <div class="upload-sub">Tippen zum Auswählen • Mehrere Bilder möglich</div>
          </div>
        </div>
      </label>

      <div class="progress"><div id="bar" class="progress-bar"></div></div>
      <p id="progressText" class="helper"></p>
    </div>

    <!-- 2️⃣ Übersicht -->
    <div class="inner-block">
      <div class="section-title">
        <h2>2️⃣ Übersicht</h2>
        <span class="badge">Live</span>
      </div>
      <div id="dashboard" class="dashboard"></div>
    </div>

    <!-- 3️⃣ Aufträge -->
    <div class="inner-block">
      <div class="section-title">
        <h2>3️⃣ Aufträge</h2>
        <span class="badge">Tabelle</span>
      </div>

      <!-- Manueller Eintrag (Accordion) -->
      <div class="section section-sub" style="margin:10px 0 0; padding:12px;">
       <div class="accordion" onclick="toggleManual()" aria-expanded="false">✍️ Manueller Eintrag</div>
	   <div id="manualContent" class="accordion-content" style="display:none">
<div class="manual-card">
  <div class="manual-grid">

    <div class="row">
      <label>Datum</label>
      <input id="m_date" type="date">
    </div>

    <div class="row">
      <label>Zeitfenster</label>
      <select id="m_time"></select>
    </div>

    <div class="row">
      <label>Bestellnummer</label>
      <input id="m_order" inputmode="numeric" placeholder="z.B. 41141243698">
    </div>

    <div class="row full">
      <label>Artikel / Beschreibung</label>
      <input id="m_artikel">
    </div>

    <div class="row">
      <label>Paket</label>
      <select id="m_package"></select>
    </div>

  </div>

  <button class="primary" onclick="addManualEntry()" style="width:100%; margin-top:10px;">
    ➕ Eintrag hinzufügen
  </button>
</div>

      </div>

<div id="workStatusFilters" class="status-filters"></div>

      <div id="tabs" class="tabs work-tabs"></div>
      <div class="table-wrap small-table">
        <table id="orderTable"></table>
      </div>
    </div>

 </div> 
</div>
</div>



<!-- ================== GUTSCHRIFT ================== -->
<div class="section section-gs">
  <div class="accordion" onclick="toggleGutschrift()" aria-expanded="false">📄 Gutschrift (PDF / XLSX)</div>

  <div id="gsContent" class="accordion-content" style="display:none">
    <input type="file" accept="application/pdf,.xlsx" onchange="importGutschrift(this.files)" style="width:100%">
	<div class="progress"><div id="gsBar" class="progress-bar"></div></div>
<p id="gsProgressText" class="helper"></p>


    <button class="danger" onclick="clearGutschrift()" style="margin-top:8px;width:100%">
      🗑 Gutschrift löschen
    </button>

    <div id="gsDashboard" class="dashboard" style="margin-top:10px;"></div>
	<div id="gsStatusFilters" class="status-filters"></div>

    <div id="gsTabs" class="tabs" style="margin-top:10px;"></div>

    <div class="table-wrap small-table">
      <table id="gsTable"></table>
    </div>
  </div>
</div>


<!-- ================== VERGLEICHSLISTE ================== -->
<div class="section section-cmp">
  <div class="accordion" onclick="toggleCompare()" aria-expanded="false">🔎 Vergleichsliste</div>

  <div id="cmpContent" class="accordion-content" style="display:none">

    <p class="helper" style="margin-top:0">
      Vergleicht <b>Aufträge</b> mit <b>Gutschrift</b> anhand der <b>Bestellnr</b> und prüft Preis-Abweichungen.
    </p>

    <button class="primary" onclick="runComparison()" style="width:100%">
      ▶ Vergleich starten
    </button>
<div class="progress"><div id="cmpBar" class="progress-bar"></div></div>
<p id="cmpProgressText" class="helper"></p>

    <div id="cmpSummary" class="dashboard" style="margin-top:10px;"></div>
    <div id="cmpTabs" class="tabs" style="margin-top:10px;"></div>

    <div class="table-wrap small-table">
      <table id="cmpTable"></table>
    </div>

  </div>
</div>
</div><!-- /.container -->
</body>
</html>